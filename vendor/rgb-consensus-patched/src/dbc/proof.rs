    ...
    Clone + Eq + Debug + StrictSerialize + StrictDeserialize + StrictDumb
{
    ...
    /// Verifies DBC proof against the provided witness transaction.
    ///
    /// The transaction is consumed through the [`WitnessTx`] trait — see
    /// that trait's docs for the minimal projection required. This makes
    /// the verification path usable against any chain whose taproot /
    /// OP_RETURN script_pubkey byte format matches Bitcoin's, including
    /// Elements / Liquid.
    ///
    /// `bitcoin::Transaction` implements `WitnessTx` directly, so
    /// existing call sites of the form `proof.verify(msg, &tx)` continue
    /// to compile unchanged.
    fn verify<W: WitnessTx>(&self, msg: &mpc::Commitment, tx: &W) -> Result<(), Self::Error>;
}
