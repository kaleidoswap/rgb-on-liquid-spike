...
impl Proof<Method> for TapretProof {
    ...
    /// key.
    ///
    /// We scan the tx's outputs for the first P2TR script_pubkey and
    /// run the SPK-level `ConvolveCommitProof` against its bytes. This
    /// is exactly what the previous `&Tx`-flavored path did internally,
    /// minus the wrapping in a `bitcoin::TxOut` shape — which means the
    /// same check now works against any `WitnessTx` implementor,
    /// including Elements/Liquid transactions.
    fn verify<W: crate::dbc::WitnessTx>(
        ...
        for spk_bytes in tx.output_script_pubkeys() {
            ...
            // First P2TR output is the tapret commitment slot.
            return ConvolveCommitProof::<_, ScriptBuf, _>::verify(self, msg, &spk);
        }
