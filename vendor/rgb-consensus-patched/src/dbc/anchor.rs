...
impl<D: dbc::Proof> Anchor<D> {
    ...
    /// that trait's docs. `bitcoin::Transaction` implements it directly,
    /// so existing call sites work unchanged. Other transaction types
    /// (e.g. Elements / Liquid) can implement `WitnessTx` to participate
    /// in this verification path.
    pub fn verify<W: WitnessTx>(
        &self,
